...
- **Multi-arch images**: The Docker image is built for **linux/amd64** and **linux/arm64** (GitHub Actions with Buildx + QEMU). Images are published to GitHub Container Registry (ghcr.io) on push to `master` or tags `v*`.
- **Dockerfile**: Multi-stage build (Node 18 bookworm builder → Node 18 Alpine final). Runs as non-root user, includes CA certs and a healthcheck. Supports build-args: `VCS_REF`, `BUILD_DATE`, `VERSION`.
...
- **Multi-arch build locally** (requires Docker Buildx):
...
>You can check out the [full license file](https://github.com/owenashurst/agar.io-clone/blob/master/LICENSE).
